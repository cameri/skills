...
const mcp = new Server(
  { name: 'telegram', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'When offering a choice or a one-tap follow-up action, phrase it as a Telegram bot command (a leading /word) instead of a plain question — Telegram renders a /command as a tappable link that sends itself back as a message when tapped, so the user doesn\'t have to type a reply. Two rules for the command text itself (user, 2026-09-13): never wrap the command in backticks or any other markup — a code span defeats the tappable-link detection and leaves the user a copy-only string to paste back by hand; and name it as the job to be done, verb first, not a description of the action (e.g. /trim_image, not /image_trim). Command names may only use lowercase letters, digits, and underscores; hyphens and spaces are not recognized as part of the command entity and just render as plain text (confirmed live 2026-08-22) — join multi-word actions with underscores, e.g. /trigger_restart, not /trigger-restart.',
      '',
