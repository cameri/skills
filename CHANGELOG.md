...
### Fixed
- **The report was invisible in dark mode, and the published page was a stale render. `finance-manager` 0.14.1.** Cameri photographed the August report on his phone: the category names and the "household" node were unreadable, and most labels "extremely dim". Both are the same bug seen twice. The page switches to a dark palette under `prefers-color-scheme: dark`, but the charts and the sankey carry their colours as SVG **presentation attributes** (`#111418` for names, `#5b6570` for dim labels), which the dark block never reached - so names rendered at a contrast ratio of **1.0:1** against `#0d1117` and dim labels at **3.1:1**. SVG attributes lose to author CSS rules, so the fix is a handful of dark-mode overrides and no change to the emitted geometry. His photograph also proved the published August page had never been re-rendered since the sankey redraw - it still contained zero node rectangles - which no hash check could have caught, because the served bytes faithfully matched the file, and the file was simply old.

### Fixed
- **The Sankey was a portrait column of curved smears. `finance-manager` 0.14.2.** Cameri, second look: "too small horizontally and the curved paths look weird." Both correct, and measuring the emitted SVG made the causes plain. The bands were cubic Bezier **strokes** at 30% opacity, so a link that dropped a long way lost its apparent thickness and neighbours merged into a smear; the middle "household" hub halved the horizontal run, giving the hub-to-destination links 30 units to cross against a stack 500 units tall; and the canvas carried a fixed pixel `height`, which caps the browser's scale factor at 1.0 - so a 420-unit drawing sat at 420px inside a 736px column on a desktop. The redraw is three changes: **filled straight ribbons** (a quad, thickness = share) instead of curved strokes; **no middle node** - income flows straight to its destinations, which doubles the run to 168 units, since a hub that everything passes through carries no information beyond being crossed; and **`height: auto` with `width: 100%`**, so the drawing fills whatever width it is given. The stack is also tighter (min_h 16->9, gap 9->5), leaving a 420x382 canvas where it was 420x630. Verified numerically rather than by eye, because this container's rasteriser has no fonts: 0 curves, 0 stroked bands, run 168 units on both sides, every ribbon within the canvas, thickness/value constant to within 0.6% above the visibility floor, and text at 9.4px on a 358px phone viewport against 19.3px on a desktop column.

### Fixed
- **`finance-manager` 0.14.3 — the docstring described the chart I had just deleted.** The 0.14.2 redraw removed the middle household node and doubled the horizontal run, but its docstring still introduced the function as "a three-column flow: income on the left, the household in the middle, the destinations on the right". Found during the immune-system review of that very release, which is the protocol doing what it is for: a comment that describes a version which no longer exists is the same defect as a stale rendered page, just written in prose instead of pixels. No behaviour change.

### Fixed
- **`finance-manager` 0.14.4 — six of fourteen flows began in empty space.** Cameri, third look at the Sankey: "some outflows don't even connect to the inflows." Correct, and it was the redraw's own doing: the left-hand ends of the links were handed out across the span of *both* columns, so as soon as the destination column grew taller than the two income bars, most ribbons started below them — the diagram was drawing money from nowhere. A sankey has exactly one invariant and this is it: **a link attaches to the node it leaves and the node it reaches, at the same thickness at both ends.** The layout now makes that structural rather than hoped for — the destination bars are sized from their values (with a floor, so a $68 destination stays visible), the income column is then given exactly that total shared out by value with no gaps, and each link takes the next run of it sized to exactly the destination it feeds. A render-time check refuses to draw the diagram if any link falls outside its source column, if a link's thickness disagrees with its destination, or if the links fail to cover the column they tile; the check is not decoration, it is the bug that shipped twice.
- **Labels were colliding** — the 9+5 pitch put the second line of one node onto the first line of the next ("Deals" printed over "Fitness"). Pitch is now 12+11, which clears a two-line label; the canvas grew to 420x588 to pay for it, which the responsive width can afford.
- Verified in the rendered SVG: 14 ribbons, 0 curved, every right end on its destination bar to the tenth of a unit, every left end inside the income column, destination pitch 23 units, all ten sections present.

### Fixed
- **`finance-manager` 0.14.5 — the new guard was decorative.** 0.14.4 added a render-time check that refuses to draw the Sankey if a link leaves its source column, and measured the links against `span_h` — the total derived from the destination column. That only restated the construction: mutating the source bars to half their height, which is the reader's defect reintroduced, passed the check silently, because `span_h` never moved. The check now measures against the bars that will actually be drawn (their own last `y + h`) and requires the source column's total to equal the links' total in both directions, not just when it is larger. Proven live by mutation: with the defect injected, the renderer refuses with *"link from y=149.5 (h=23.0) falls outside the source column 46.0..149.5; the diagram would draw money from nowhere."* The rendered page is byte-identical - the fix changes nothing except whether a wrong diagram can be published.
